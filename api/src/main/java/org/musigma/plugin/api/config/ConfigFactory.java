...
import org.musigma.plugin.api.ConfigNotSupportedException;
...
public interface ConfigFactory {
    ...
    static ConfigNode loadFromContextLoader(final String name) throws IOException {
        return load(Thread.currentThread().getContextClassLoader(), ConfigSource.fromContextLoader(name));
    }

    static ConfigNode load(final ClassLoader loader, final ConfigSource source) throws IOException {
    ...
    ConfigNode load(ConfigSource source) throws IOException;
